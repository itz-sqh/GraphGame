...
void Obstacle::draw(sf::RenderTarget &target) const {
    ...
    float windowRadius = Geometry::mapToWindow(m_radius + GameConstants::MIN_X, size.x);
...
void Obstacle::addOverlap(sf::Vector2f position) {
    m_overlaps.emplace_back(position, GameConstants::HIT_RADIUS);
}
